...
object SQLDateEncodersDecoders {
  ...
  implicit val encodeDateTime =
    MappedEncoding[SharedDateTime, String](
      d =>
        SharedDateDecoders
          .sharedDTToDT(d)
          .toString(formatterDateTime)
    )
  ...
  implicit val encodeDate =
    MappedEncoding[SharedDateOnly, String](
      d =>
        new DateTime(d.millis).toString(formatterDateOnly)
    )

  implicit val decodeDate =
    MappedEncoding[String, SharedDateOnly](
      x =>
        formatterDateOnly
          .parseDateTime(x)
          .toSharedDateOnly()
  ...
  implicit val encodeTime =
    MappedEncoding[SharedTimeOnly, String](
      d =>
        SharedDateDecoders
          .sharedTimeOnlyToDT(d)
          .toString("HH:mm:ss")
    )

  implicit val decodeTime =
    MappedEncoding[String, SharedTimeOnly](
      d => formatterTime.parseDateTime(d).toSharedTimeOnly()
    )
